...
{ //TO-DO: se hace con el programa heracles???
  uint32_t vaddr, paddr, instruction, opcode;

  vaddr = pcb->mm->code + pcb->executedCycles * WORD_SIZE;
  paddr = translate_vaddr_to_paddr(pcb, vaddr); 
  instruction = read_word(paddr);
  opcode = instruction & 0xF00000; //obetener el bit correspondiente a la operacion
  
  //TO-DO: obtener los registros a utilizar en la operacion
  //en las operaciones de LD y ST obtener la direccion virtual indicada y traducirla a direccion fisica
  /*if (opcode == LOAD)
  {
  ...
  {
    printf("ERROR (execute_process): instruccion desconocida en el proceso %d: 0x%x\n", pcb->pid, instruction);
    return;
  }*/
